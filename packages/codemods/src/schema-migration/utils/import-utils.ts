...
export function transformModelToResourceImport(
  ...
  if (shouldImportFromTraits(relatedType, options)) {
    ...
    if (traitsImport) {
      return `import type { ${traitInterfaceName} as ${aliasName} } from '${traitsImport}/${relatedType}.schema'`;
    } else {
      return `import type { ${traitInterfaceName} as ${aliasName} } from '../traits/${relatedType}.schema'`;
    }
  }
