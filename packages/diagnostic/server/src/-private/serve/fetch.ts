...
export async function handleFetch(config: LaunchConfig, state: LaunchState, c: Context): Promise<Response> {
  ...
  info(`[${styleText('cyan', c.req.method)}] ${url.pathname}`);
  ...
  if (INDEX_PATHS.includes(url.pathname)) {
    ...
    const STATIC_ASSETS: Record<string, string> = {
      ...
      'NCC-1701-a-gold_100.svg': 'image/svg+xml',
      'prefers-color-w.svg': 'image/svg+xml',
      'logo-yellow-slab.svg': 'image/svg+xml',
    };
    if (route in STATIC_ASSETS) {
      const asset = path.join(__dirname, route);

      try {
        const content = await readFile(asset);
        return new Response(content, {
          headers: { 'Content-Type': STATIC_ASSETS[route] },
        });
      } catch {
