...
description: Where to find llms.txt, llms-full.txt, llms-legacy.txt, and the plain-Markdown twin of every page, and how the Copy page button uses them.
---
...
- [llms-legacy.txt](/llms-legacy.txt) is the same kind of index for the legacy `@ember-data/*`
  and early `@warp-drive/*` packages, which the two files above leave out. Use it only for an app
  that still imports from those packages.
