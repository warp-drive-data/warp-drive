...
/**
 * How the cache decides whether an attribute value changed (#10094). A `schema-object`, and each
 * element of a `schema-array`, compares by the identity hash its `ObjectSchema` declares, so the
 * schema owns the definition of "same". Everything else, including a schema-object with
 * `identity: null` and the unstructured `object` and `array` kinds, compares by reference and so
 * notifies on every re-push of a fresh payload. That over-notification is by design: content
 * equality is the schema's to define, not the cache's to guess.
 *
 * Assert on notifications, never on a subsequent property read: a getter re-reads the cache, so
 * reading the value back passes whether or not anything was notified.
 */
...
module<CustomContext>('Integration | <JSONAPICache> hash equality of schema-object values', function (hooks) {
