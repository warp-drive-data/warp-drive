...
export default class extends Route {
  ...
  async model() {
    ...
    const changedPayload = await fetch('./fixtures/json-records-changed.json').then((r) => r.json());
    ...
    performance.mark('start-push-changed-payload');
    this.store.push(changedPayload);

    performance.mark('end-push-changed-payload');
  }
}
