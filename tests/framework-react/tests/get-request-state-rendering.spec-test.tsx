...
GetRequestStateRenderingSpec.use(useReact(), function (b) {
  b.test("it renders each stage of a request resolving in a new microtask queue", function (props) {
    ...
    function Component() {
      ...
      if (state.isSuccess && state.result) {
        ...
        if (result.data?.attributes?.name) {
          return (
            <>
              {result.data.attributes.name}Count: <CountFor countFor={countFor} data={state.result} />
            </>
          );
        }
      }
    ...
    .test("it transitions to error state correctly", function (props) {
      ...
      function Component() {
        ...
        if (state.isPending) {
          return (
            <>
              Pending Count: <CountFor countFor={countFor} data={state.result} error={state.error} />
            </>
          );
        }
        if (state.isError && state.error) {
          return (
            <>
              {state.error.message} Count: <CountFor countFor={countFor} data={state.result} error={state.error} />
            </>
          );
        }
