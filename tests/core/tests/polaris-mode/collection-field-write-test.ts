...
module('Writes | collection', function (hooks) {
  ...
  test('we can mutate the data array of a checked out record', async function (assert) {
    ...
    assert.arrayEquals(ids(editable4.friends), ['1'], 'the local inverse is updated');
    assert.arrayEquals(ids(record4.friends), [], 'the immutable inverse reflects the remote state');
    const editable5 = await checkout<User>(record5);
    assert.arrayEquals(ids(editable5.friends), [], 'the local inverse is removed');
