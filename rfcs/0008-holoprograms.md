---
title: HoloPrograms
description: A HoloProgram answers every request a holodeck test makes from a per-test WarpDrive store while recording, so a test starts from a seeded scene instead of declaring one mock per request.
warp-drive-rfc: 8
emberjs-rfc:
emberjs-pr:
emberjs-branch:
...
<!--
DECISION: a bodiless mock helper records whatever its response function returned, not `{}`, so
this default breaks the "shape a mock helper records" claim for empty bodies, and a 204 fixture
would carry a `{}` body. Options: record `null` and have the server write an empty body for it, or
keep `{}` and drop the byte-for-byte claim for 204s.
-->
...
forwarded request that tells the server to skip the wait, which adds API. Pick one and update the
open question on delays to match.
-->
