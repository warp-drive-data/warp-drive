...
export default [
  ...
  typescript.browser({
    ...
    // longer needs to run the type-aware rules oxlint now owns for these files. `.gts` files
    // are handled by the separate `gts.browser()` block below, which keeps full type-aware
    // ESLint coverage since oxlint's parser can't scan those.
