...
import type { LegacyQueryArray } from './record-arrays/legacy-query.ts';
...
//
// A second, separate `export interface Store {}` (TypeScript merges
// same-named interfaces declared within a module) rather than a single
// merged block with the interface above, so this can be found and removed
// as its own unit.
//
// These signatures used to live in `../deprecated/store.ts` as a cross-file
// `declare module '../-private/store-service' { interface Store {...} }`
// augmentation. That stopped working once `Store` began being emitted
// through an entry-based, rolled-up `.d.ts` build (tsdown/rolldown-plugin-dts):
// `Store` gets hoisted into a shared, hash-named chunk whenever 2+ entries
// reference it (which is always), and a cross-file module augmentation can't
// target an unstable chunk name -- it silently fails to attach for any
// consumer not importing through the exact one path the augmentation happened
// to specify, which in practice is every real consumer. Same-module interface
// merging has no such problem, since it's resolved before any bundling happens.
//
// The runtime implementations of these methods still live in
// `../deprecated/store.ts`, gated behind the `ENABLE_LEGACY_REQUEST_METHODS`
// deprecation flag. Delete this interface (and the `Store` import it drops
// from `../deprecated/store.ts`'s `declare module` block) once those methods
// are fully removed.
/////////////////////////////////////////////////////////////////////////////
export interface Store {
  /**
